...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Insert into our custom User table for extra profile data
      // Note: In a production app, you'd use a Supabase Trigger for this
      ...
      <PublicLayout>
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="max-w-md w-full text-center space-y-6 animate-in fade-in zoom-in duration-500">
            ...
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">{t("checkYourEmail")}</h1>
            <p className="text-slate-500 font-medium leading-relaxed">
              {t("verificationLinkSent")} <span className="text-primary font-bold">{email}</span>. 
            </p>
            <Link 
              href="/login" 
              className="inline-flex items-center gap-2 text-primary font-bold hover:underline"
            >
              {t("backToLogin")} <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </div>
      </PublicLayout>
    ...
    <PublicLayout hideNav={true}>
      ...
      <div className="min-h-screen flex flex-col lg:flex-row bg-slate-900 lg:bg-white overflow-hidden">
          ...
          <div className="relative z-10 w-full p-16 flex flex-col justify-between">
            ...
            <div className="space-y-6">
              <h2 className="text-5xl font-bold text-white leading-tight tracking-tight">
                ...
                <span className="text-primary">With YSG.</span>
              </h2>
              <p className="text-slate-300 text-lg max-w-md font-medium leading-relaxed">
                Create an account to track your orders, get exclusive machinery deals, and connect with industry experts.
              </p>
            </div>
        ...
        <div className="w-full lg:w-1/2 flex flex-col relative bg-slate-900 lg:bg-white">
          ...
          <div className="lg:hidden h-[20vh] flex flex-col items-center justify-center space-y-4 px-6 text-center animate-in fade-in slide-in-from-top-10 duration-700">
            ...
            <div className="max-w-md w-full space-y-8">
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                <div className="space-y-3">
                  <div className="group">
                    ...
                    <div className="relative">
                      <User className="absolute left-5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-300 group-focus-within:text-primary transition-colors" />
                      <input
                        type="text"
                        required
                        value={fullName}
                        onChange={(e) => setFullName(e.target.value)}
                        className="w-full pl-14 pr-6 py-3.5 bg-slate-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-primary/20 outline-none transition-all font-semibold text-slate-900 placeholder:text-slate-300"
                  ...
                  <div className="group">
                    ...
                    <p className="text-[9px] text-slate-400 ml-1 mt-1 font-bold uppercase tracking-widest">{t("passwordRequirement")}</p>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-[0.25em] text-xs hover:bg-primary hover:shadow-2xl hover:shadow-primary/30 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-4 mt-6"
                >
                  {loading ? (
