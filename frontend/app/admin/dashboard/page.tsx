  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-medium text-slate-900 tracking-tight">{t("dashboard")}</h1>
          <p className="text-sm text-slate-500 mt-1">{t("welcomeBack")}</p>
        </div>
        <div className="flex items-center gap-3">
          <Link 
            href="/admin/products/new" 
            className="flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-medium text-sm hover:bg-slate-50 transition-all shadow-sm"
          >
        ...
        {statCards.map((card, idx) => (
          <div key={idx} className={`p-6 bg-white rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md group`}>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-[10px] font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg">
                <ArrowUpRight className="w-3 h-3" />
                +12%
              </div>
            </div>
            <p className="text-xs font-medium text-slate-400 uppercase tracking-widest">{card.title}</p>
            <h3 className="text-2xl font-medium text-slate-900 mt-1">{card.value}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <h2 className="text-lg font-medium text-slate-900">{t("recentOrders")}</h2>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="px-6 py-4 text-[10px] font-medium text-slate-400 uppercase tracking-widest">{t("orderId")}</th>
                  <th className="px-6 py-4 text-[10px] font-medium text-slate-400 uppercase tracking-widest">{t("customer")}</th>
                  <th className="px-6 py-4 text-[10px] font-medium text-slate-400 uppercase tracking-widest">{t("amount")}</th>
                  <th className="px-6 py-4 text-[10px] font-medium text-slate-400 uppercase tracking-widest text-right">{t("status")}</th>
                </tr>
              </thead>
                ...
                {recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/50 transition-all">
                    <td className="px-6 py-4">
                      <span className="text-xs font-medium text-slate-900">#{order.id.slice(0, 8)}</span>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-xs font-medium text-slate-700">{order.customerName || t("walkInCustomer")}</p>
                      <p className="text-[10px] text-slate-400 font-medium">{new Date(order.createdAt).toLocaleDateString()}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-xs font-medium text-slate-900">{formatPrice(order.totalAmount)}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className={`text-[10px] font-medium px-2.5 py-1 rounded-lg uppercase tracking-wider ${getStatusColor(order.status)}`}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col">
          <h2 className="text-lg font-medium text-slate-900 mb-6">{t("quickActivity")}</h2>
          ...
          <button className="w-full mt-auto py-3 rounded-xl border border-slate-100 text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em] hover:bg-slate-50 transition-all">
            {t("seeAllActivity")}
          </button>
        </div>
      </div>
