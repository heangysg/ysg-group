...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
 ...
 <div className="w-9 h-9 bg-white rounded-md flex items-center justify-center shadow-sm border border-slate-200 overflow-hidden">
 <img src="/logo.png" alt="YSG Logo" className="w-7 h-7 object-contain" />
 </div>
 <div>
 <h1 className="text-sm font-bold text-white leading-tight uppercase">
 ...
 ${isActive
 ? "bg-primary text-white border-transparent mx-2 rounded-md"
 : "text-slate-600 border-transparent hover:text-slate-900 hover:bg-slate-50 mx-2 rounded-md"}
 `}
 >
 ...
 <div className={`shrink-0 w-10 h-10 rounded-md ${note.bg} ${note.color} flex items-center justify-center`}>
 <note.icon className="w-5 h-5" />
 </div>
 <div className="flex-1 min-w-0">
 <p className="text-xs font-medium text-slate-900 truncate">{t(note.titleKey || note.title)}</p>
 <p className="text-xs text-slate-500 mt-0.5 line-clamp-2 leading-relaxed">
    {note.type === 'order' 
      ? `${t("order")} #${note.orderId} - $${note.amount}`
      : (note.rawMessage?.slice(0, 50) + (note.rawMessage?.length > 50 ? '...' : ''))}
  </p>
